...
static int __init char_driver_init (void)
{
	...
	dev->size = MAX_SIZE;
	dev->kmem_c = kmem_cache_create("exp_lookaside", dev->size,
					0,
					SLAB_HWCACHE_ALIGN,
					NULL
					);
        ...
        {
                prv_ptr = kmem_cache_alloc(dev->kmem_c, GFP_KERNEL);
                if(!prv_ptr){
                        printk(KERN_ALERT "Unable to allocate memory\n");
                        kmem_cache_destroy(dev->kmem_c);
                        return -1;
                }
        }
	dev->prv_ptr = prv_ptr;
...
{
	kmem_cache_free(dev->kmem_c, dev->prv_ptr);
        kmem_cache_destroy(dev->kmem_c);
	cdev_del(&dev->cdev);
