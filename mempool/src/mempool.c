...
static int __init char_driver_init (void)
{
	...
	/*
 	 * We will use mempool of flavour kmem_cache interface,
 	 * for other interface flavour look into linux/mempool.h
 	 */

	dev->kmem_t = kmem_cache_create("exp_mempool", dev->size,
					0,
					SLAB_HWCACHE_ALIGN,
					NULL
					);
        ...
        if(!dev->kmem_t){
                printk(KERN_INFO "Failled to create cache\n");
                return -1;
        }
        else
        {
		dev->mem_t = mempool_create( BUFFER_NUMS,
					     mempool_alloc_slab,
				    	     mempool_free_slab,
				    	     dev->kmem_t
				    	    );
		if(dev->mem_t){
			prv_ptr = mempool_alloc( dev->mem_t, GFP_KERNEL);
		}
		else
		{
        		kmem_cache_destroy(dev->kmem_t);
			return -1;
		}
        }
	dev->prv_ptr = prv_ptr;
...
{
	mempool_free(dev->prv_ptr, dev->mem_t);
	mempool_destroy(dev->mem_t);
        kmem_cache_destroy(dev->kmem_t);
	cdev_del(&dev->cdev);
