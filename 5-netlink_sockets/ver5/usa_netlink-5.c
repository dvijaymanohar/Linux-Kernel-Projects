...
receive:
  ...
  if (pl_len)
  {
    *nl_data = malloc(sizeof(struct nl_mesg));
...
int main()
{
  ...
  while (1)
  {
    ...
    //printf("Recieved message: %u\n\n", msg_rx->data->cmd_hdr1.opcode);
    //printf("Recieved message: %u\n\n", msg_rx->data->cmd_hdr2.param_tot_len);
    ...
    //free(msg_rx->data); Free the incoming buff
    free(msg_rx);
  }
