...
static int fmr_hci_write_cmd_assembly(u8 *cmd_buff, T_FMR_HCI_CMD fmr_hci_cmd,
                                      u8 plen)
{
  int index = 0;
...
static void fmr_hci_store_cmdhdr1(T_FMR_HCI_CMD fmr_hci_cmd, u8 *cmd_buff)
{
                                ...
                                u8 data_plen /* data length */ )
{
  ...
  int rc = 0;
	...
	//lnp_fmdrv_dbg("Func: %s Line : %d data_plen : %d\n", __FUNCTION__, __LINE__, data_plen);

	switch(fmr_hci_cmd.cmd_hdr1.opcode)
  {
    case HCI_INTEL_FMR_WRITE:
    {
      cmd_buff = (u8 *) kzalloc(FMR_HCI_WRITE_CMD_LEN + data_plen + 2, GFP_KERNEL);

      if (NULL == cmd_buff)
      {
        printk("Could not allocate mem : %d bytes\n", FMR_HCI_WRITE_CMD_LEN + data_plen + 2);
        return -ENOMEM;
      }

     	fmr_hci_store_cmdhdr1(fmr_hci_cmd, cmd_buff);

			rc = fmr_hci_write_cmd_assembly(cmd_buff, fmr_hci_cmd, data_plen);
    }
    break;

    case HCI_INTEL_FMR_READ:
    {
      cmd_buff = (u8 *) kzalloc(FMR_HCI_READ_CMD_LEN + data_plen + 2, GFP_KERNEL);

      if (NULL == cmd_buff)
      {
        printk("Could not allocate mem : %d bytes\n", FMR_HCI_READ_CMD_LEN + data_plen + 2);
        rc = -ENOMEM;
        goto fmr_hci_cmd_assembly_alloc_err;
      }

     	fmr_hci_store_cmdhdr1(fmr_hci_cmd, cmd_buff);

  		rc = fmr_hci_read_cmd_assembly(cmd_buff, fmr_hci_cmd);
    }
    break;

    case HCI_INTEL_FMR_SET_POWER:
		{
      cmd_buff = (u8 *) kzalloc(FMR_HCI_SET_POW_CMD_LEN + data_plen + 2, GFP_KERNEL);

      if (NULL == cmd_buff)
      {
        printk("Could not allocate mem : %d bytes\n", FMR_HCI_SET_POW_CMD_LEN + data_plen + 2);
        rc = -ENOMEM;
        goto fmr_hci_cmd_assembly_alloc_err;
      }
     	...
     	fmr_hci_store_cmdhdr1(fmr_hci_cmd, cmd_buff);
			...
			rc = fmr_hci_set_pow_cmd_assembly(cmd_buff, fmr_hci_cmd);
    }
    break;

    case HCI_INTEL_FMR_SET_AUDIO:
    {
      cmd_buff = (u8 *) kzalloc(FMR_HCI_SET_AUD_CMD_LEN + data_plen + 2, GFP_KERNEL);

      if (NULL == cmd_buff)
      {
        printk("Could not allocate mem : %d bytes\n", FMR_HCI_SET_AUD_CMD_LEN + data_plen + 2);
        return -1;
      }

     	fmr_hci_store_cmdhdr1(fmr_hci_cmd, cmd_buff);

			rc = fmr_hci_set_aud_cmd_assembly(cmd_buff, fmr_hci_cmd);
    }
    break;

    case HCI_INTEL_FMR_IRQ_CONFIRM:
    {
      cmd_buff = (u8 *) kzalloc(FMR_HCI_CONFIRM_IRQ_CMD_LEN + data_plen + 2, GFP_KERNEL);

      if (NULL == cmd_buff)
      {
        printk("Could not allocate mem : %d bytes\n", FMR_HCI_CONFIRM_IRQ_CMD_LEN + data_plen + 2);
        return -1;
      }

     	fmr_hci_store_cmdhdr1(fmr_hci_cmd, cmd_buff);

      rc = fmr_hci_confirm_irq_cmd_assembly(cmd_buff, fmr_hci_cmd);
    }
    break;
  ...
  printk("%s: Freeing the commd buff\n", __FUNCTION__);
  kfree(cmd_buff);
  cmd_buff = NULL;

  return rc;

fmr_hci_cmd_assembly_alloc_err:
  return rc;
}
