...
static int send_netlink(struct nl_soc    *nl_socket,
                        ...
                        T_FMR_HCI_CMD_COMPLETE_EV *nl_data,
			                  u8  msg_type,
       				          u16 msg_flags,
              			    u32 msg_seq)
{
  ...
  if (payload_len)
  {
    ...
    memcpy(NLMSG_DATA(nlh), nl_data, payload_len);
  }
...
int main(int argc, char *argv[])
{
  ...
  T_FMR_HCI_CMD_COMPLETE_EV msg_tx;
  ...
  while (1)
  {
    ...
    getchar();
		 ...
		 /* Form the event response packet to LK NL */
		 msg_tx.cmd_ev_hdr1.event_code    = FMR_HCI_EVENT_PACKET;
		 msg_tx.cmd_ev_hdr2.param_tot_len = 4;
		 msg_tx.cmd_ev_hdr2.params.fmr_hci_write_cmd_complete.num_hci_cmd_packets = 0; /* VMD Todo: ?? */
		 msg_tx.cmd_ev_hdr2.params.fmr_hci_write_cmd_complete.cmd_opcode = HCI_INTEL_FMR_WRITE;
		 msg_tx.cmd_ev_hdr2.params.fmr_hci_write_cmd_complete.status     = FMR_HCI_CMD_EXEC_STATUS;
