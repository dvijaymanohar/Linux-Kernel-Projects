...
int our_probe(struct pci_dev *pci,  const struct pci_device_id *snd_cs4280_id)
{
	...
	printk(KERN_ALERT "This is Debug sentence and pci is %p\n", pci);
	pci = pci_find_device(PCI_VENDOR_OUR_ID, PCI_DEVICE_OUR_ID, from);
	if(pci != NULL)
	{
		printk(KERN_ALERT "From is %p\n", from);
		printk(KERN_ALERT "This ROCKS %p\n" , pci);
		return 0;
	}
	else
		printk(KERN_ALERT "Oops...YEAH BABE %p\n", pci);
	return -1;
}
