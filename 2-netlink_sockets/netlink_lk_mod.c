/*
Program Description :: A thread based kernel module which main purpose is to
send the ping packets from kernel space to User space via netlink socket. (
keep default ping duration i.e.1Sec)
*/
...
static spinlock_t send_hello_lock;
static spinlock_t send_bye_lock;
                              ...
                              const struct net_device *outdev)
{
  ...
  if (iph == NULL)
  {
    printk(KERN_CRIT"bye_IPv4_in_hook :: Failed to decode ip
           header :: drop the pkt\n");
    return NF_DROP;
  }
...
static int send_bye_response(void *msg, int size)
{
  ...
  {
    printk(KERN_CRIT"Hello >> Comunication between U-Plane and C-Plane
           closed\n");
    goto nlmsg_failure;
  }
...
{
  hello_thread_id = kernel_thread(process_hello_requests, NULL, CLONE_FS |
                                  CLONE_FILES | CLONE_SIGHAND | SIGCHLD | CLONE_KERNEL);
...
{
  bye_thread_id = kernel_thread(process_bye_requests, NULL, CLONE_FS |
                                CLONE_FILES | CLONE_SIGHAND | SIGCHLD | CLONE_KERNEL);
