/*
 * Char driver for 2.6
 */
#include <linux/kernel.h>			/* printk */
#include <linux/init.h>				/* module_init, module_exit */
#include <linux/module.h>
#include <linux/moduleparam.h>
#include <linux/types.h>			/* dev_t */
#include <linux/fs.h>				/* register_chr... */
#include <linux/cdev.h>

#include <linux/slab.h>				/* kmalloc */
#include <linux/interrupt.h>			/* IRQF.... */
//#include <linux/signal.h>			/* SA_... */
#include <linux/sched.h>			/* pulls jiffies.h> */
#include <linux/time.h>				/* jiffies_to_timeval */
#include <linux/workqueue.h>

#include <asm/atomic.h>
#include <asm/uaccess.h>			/* copy_to_user.... */
//#include <asm/signal.h>			/* SA_... */


#define MAX_SIZE		(128 * 1024)


/*
 * MY Device Structure
 */

struct exp_dev {
	void * prv_ptr;
	int size;
	dev_t dev_no;
	int irq_line;

	/*
         *Embed cdev in device structure, so that one can
	 *get back pointer to device structure using container_of macro
	 */ 
	struct cdev cdev;

	int devices;
	atomic_t irq_cnt;

	/* who is first? */

	struct tasklet_struct tsk;
	atomic_t tsk_cnt;

	struct work_struct	work; /* shared work queue */
};

struct exp_dev	exp_dev;
struct exp_dev *dev = &exp_dev;

/*
 * Demo irq handler
 */

irqreturn_t demo_handler(int irq_line, void *dev_id, struct pt_regs *regs)
{
	struct exp_dev * dev = dev_id;
	int irq_cnt;
//	u64 j;


	/* For shared interrupt line,
 	 * actual implementation is to check interrupt status register of
 	 * device and make sure wheather interrupt is from our device. If
 	 * not from our device return IRQ_NONE immediately.
 	 */

	irq_cnt = atomic_inc_return(&dev->irq_cnt);

	if( (irq_cnt % 200) != 0){
		/* do nothing */
		return IRQ_NONE;
	}
	else
	{
		atomic_inc(&dev->tsk_cnt);
	}

	/*
 	 * Schedule Bottom half using work queue
 	 */
	schedule_work(&dev->work);

	/*
 	 * Schedule Bottom half using tasklet
 	 */
	tasklet_schedule(&dev->tsk);
	return IRQ_HANDLED;

}


/*
 * Tasklet function
 */

void demo_tsk(unsigned long data)
{
	printk(KERN_INFO "Executing bottom half of irq using tasklet\n");
	if( atomic_dec_return(&dev->tsk_cnt) > 0){
		tasklet_schedule(&dev->tsk);
	}
	return;
}

/*
 * Work queue
 */

void demo_wrk( void)
{
	printk(KERN_INFO "Executing bottom half of irq\
					 using shared workqueue\n");
	if( atomic_dec_return(&dev->tsk_cnt) > 0 ){
		schedule_work( &dev->work);
	}	
	return;
}
	
/*
 * read
 */
ssize_t exp_read( struct file *file_ptr, char __user * user_buf,
		   size_t count, loff_t *f_ops)
{
	return 0;
}

/*
 * write
 */ 
ssize_t exp_write( struct file *file_ptr, const char __user * user_buf,
		   size_t count, loff_t *f_ops)
{
	return 0;
}

/*
 * open
 */ 
int exp_open(struct inode *inode, struct file *file_ptr)
{
	return 0;
}

/*
 * release
 */ 
int exp_release(struct inode *inode, struct file *file_ptr)
{
	return 0;
}

/*
 * llseek
 */ 
loff_t exp_llseek(struct file *file_ptr, loff_t off, int whence)
{
	return  0;
}

/*
 * ioctl
 */ 
int exp_ioctl (struct inode *inode, struct file *file_ptr, unsigned int cmd,
		unsigned long arg)
{
	return 0;
}

/*
 * Char Interface for Device
 */

struct file_operations exp_fops = {
	.owner   = THIS_MODULE,
	.read    = exp_read,
	.write   = exp_write,
	.llseek  = exp_llseek,
	.ioctl   = exp_ioctl,
	.open    = exp_open,
	.release = exp_release,
};

static int __init char_driver_init (void)
{
	int major = 0,minor = 1;
	int err = 0;
	void *prv_ptr;
	dev_t dev_no;
	unsigned long int data = 0;

	dev->devices = 1;
	atomic_set(&dev->tsk_cnt, 0);

	/*
 	 * Upper limit for kmalloc is 128 KB
 	 * if your code need to be complete portable
 	 */
	dev->size = MAX_SIZE;
	prv_ptr = kmalloc( MAX_SIZE, GFP_KERNEL );
	if( prv_ptr == NULL ){
		printk(KERN_ERR "Unable to allocate memory\n");
		return -1;
	}
	dev->prv_ptr = prv_ptr;
 
	if(major){
		dev_no = MKDEV(major, minor);
		if( register_chrdev_region(dev_no,
					   dev->devices, "MY_CHAR_DEV" ) < 0 )
			return -1;
	}
	else
	{
		if( alloc_chrdev_region(&dev_no, minor, 
					dev->devices, "MY_CHAR_DEV") < 0)
			return -1;
	}
	dev->dev_no = dev_no;

	cdev_init(&dev->cdev, &exp_fops);
	dev->cdev.owner = THIS_MODULE;
	dev->cdev.ops = &exp_fops;

	err = cdev_add(&dev->cdev, dev_no, dev->devices);
	if( err ){
		printk(KERN_NOTICE "Error Adding the char driver\n");
		return -1;
	}
	printk(KERN_INFO "Char driver, major no %d, minor no %d registered\n",\
		MAJOR(dev_no), MINOR(dev_no));

	/*
 	 * Intialize tasklet
 	 */
	tasklet_init(&dev->tsk, demo_tsk, data);

	/*
	 * Intialize shared work queue
	 */

//	INIT_WORK(&dev->work, demo_wrk, &data);
	INIT_WORK(&dev->work, demo_wrk);
	
	/* 
 	 * Experimental Purpose,
 	 * Inspected /proc/interrupts to get shared interrupt line info
 	 * don't stay in handler. Selected irq line 19 (more interrupts)
 	 */

	dev->irq_line = 19;

	err = request_irq(dev->irq_line, demo_handler,
//			  SA_INTERRUPT | SA_SHIRQ,
			  IRQF_DISABLED | IRQF_SHARED,
			  "irq_exp", (void *)dev);

	if( err < 0 ){
		printk(KERN_INFO "irq_exp: unable to get irq %d\n", dev->irq_line);
		dev->irq_line = -1;
	}
	
	return 0;
}

void __exit char_driver_cleanup (void)
{
	tasklet_kill(&dev->tsk);

//	cancel_delayed_work(&dev->work);
	flush_scheduled_work();

	if( dev->irq_line != -1 ){
		free_irq(dev->irq_line, (void *)dev);
	}
	
	kfree(dev->prv_ptr);
	cdev_del(&dev->cdev);
	unregister_chrdev_region(dev->dev_no, dev->devices);
}

module_init(char_driver_init);
module_exit(char_driver_cleanup);

MODULE_AUTHOR("Padmanabha S");
MODULE_DESCRIPTION("SIMPLE CHAR DRIVER IMPLEMENTATION");
MODULE_LICENSE("GPL");

