...
int main()
{
	...
	unsigned int return_value, pattern_option, direct_option, repeat_option;
	...
	while(1)
	{
		...
		printf("|0 - 1 time\t ~through~\t15 - 16  times|\n(Enter 81 to exit)\nYour option: ");
...
{
	if(option == 0)
		printf("-=- Right-to-Left direction is selected for the pattern movement.\n");
	else if(option == 1)
		printf("-=- Left-to-Right direction is selected for the pattern movement.\n");
	else
		return -1;
	return 0;
}
