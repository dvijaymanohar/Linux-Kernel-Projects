...
ssize_t my_write(struct file *fd, const char __user * buf, size_t len, loff_t *off)
{
	...
	unsigned long ret = 0;
	unsigned char mask = 0x00, clone_pattern,  repeat_count, counter, move;
	size_t length = 8;
	
	ret = copy_from_user(&led_char, buf, 1);
	printk(KERN_DEBUG "The value of led_char is %x \n", led_char);
	pattern = select_pattern(led_char);
	repeat_count = led_char & 0x0F;
	clone_pattern = pattern;
	printk(KERN_DEBUG "The selected pattern is %x \n", pattern);
	for(counter = 0; counter <= repeat_count; counter++)
	{
		for(move = 0; move <= LED_BITS; move++)
		{
